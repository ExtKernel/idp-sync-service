...
(
    ...
    principal_username VARCHAR(255),
    principal_password VARCHAR(255),
    CONSTRAINT pk_synckcclient PRIMARY KEY (id)
);

CREATE TABLE sync_kc_client_refresh_tokens
(
...
ALTER TABLE sync_kc_client_refresh_tokens
    ADD CONSTRAINT uc_sync_kc_client_refresh_tokens_refreshtokens UNIQUE (refresh_tokens_id);
...
ALTER TABLE sync_kc_client_refresh_tokens
    ADD CONSTRAINT fk_synkcclireftok_on_refresh_token FOREIGN KEY (refresh_tokens_id) REFERENCES token (id);

ALTER TABLE sync_kc_client_refresh_tokens
    ADD CONSTRAINT fk_synkcclireftok_on_sync_kc_client FOREIGN KEY (sync_kc_client_id) REFERENCES sync_kc_client (id);
